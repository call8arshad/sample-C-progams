...
int main(int argc , char *argv[])
{
    ...
    if( (master_socket = socket(AF_INET , SOCK_DGRAM, /* SOCK_STREAM ,*/ 0)) == 0) 
    {
        perror("socket failed");
        exit(EXIT_FAILURE);
    }

    /* set master socket to allow multiple connections , this is
       just a good habit, it will work without this */
